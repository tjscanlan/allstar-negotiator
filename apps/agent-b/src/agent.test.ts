...
describe("agent-b", () => {
  ...
  test("proposeOpeningTerms requests a submit_proposal tool call with the Agent B prompt", async () => {
  ...
  test("reactToProposal restates own history and the counterparty's last proposal", async () => {
    const counter = modelOutput({ agentId: "agent-a", round: 1, rationale: "their move" });
    const history = [modelOutput({ agentId: "agent-b", round: 1, rationale: "my first move" })];
  ...
  test("system prompt states the 40% floor", () => {
    expect(SYSTEM_PROMPT).toContain("Your minimum acceptable share is 40%");
